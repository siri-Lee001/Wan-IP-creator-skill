#!/usr/bin/env python3
# 万相IP技能 - 第一步：生成角色三视图（简单版）
# 使用Wan-skills内置的图片压缩功能
...
def main():
    ...
    # 使用适配器的图片压缩功能
    print("\n使用Wan-skills内置图片压缩功能...")
    try:
        data_url = adapter.compress_image_to_base64(input_image_path)
        if data_url:
            print(f"图片压缩成功，Base64长度: {len(data_url)} 字符")
            if len(data_url) > 61440:
                print(f"警告: Base64长度超过API限制(61440)，但Wan-skills可能支持")
        else:
            print("图片压缩失败")
            return {
                "status": "error",

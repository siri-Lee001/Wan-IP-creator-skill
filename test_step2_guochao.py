...
def main():
    ...
    # 国潮风格提示词（三视图输入时的附加要求）
    prompt = """转换为国潮艺术风格，传统中国风，水墨渲染效果，传统色彩搭配，生成单一角色正面主体图，去掉三视图的多视图布局，只保留单个完整的角色，100%保留角色核心视觉特征。"""
    
    print(f"提示词长度: {len(prompt)} 字符")
    print("正在调用万相API生成国潮风格图...（预计耗时30-60秒）")
    ...
    if result and len(result) > 0:
        ...
        output_path = os.path.join(output_dir, "giraffe_guochao_style.jpg")

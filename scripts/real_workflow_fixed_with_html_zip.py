...
# -*- coding: utf-8 -*-
"""
真实工作流脚本 - 修复版：添加HTML生成和文件打包节点
最小改动原则：只添加缺失功能，不改变现有正确代码
"""
...
class RealWorkflowFixed:
    ...
    def __init__(self, role_image_path: str, role_name: str = "角色", output_dir: str = "real_output", 
        ...
        os.makedirs(os.path.join(output_dir, "html"), exist_ok=True)  # HTML输出目录
        os.makedirs(os.path.join(output_dir, "packages"), exist_ok=True)  # 打包目录
...
要求：单张大图，整齐排列，每个表情清晰可辨，带中文文字，保留角色核心特征。""",
            ...
            # 新增步骤7：HTML生成
            {
                "id": 7,
                "name": "HTML展示页面生成",
                "description": "生成展示所有结果的HTML页面",
                "type": "html_generation",  # 特殊类型
                "filename": "index.html"
            },
            # 新增步骤8：文件打包
            {
                "id": 8,
                "name": "文件打包",
                "description": "将所有输出文件打包成ZIP文件",
                "type": "packaging",  # 特殊类型
                "filename": f"万相IP技能成果包_{datetime.now().strftime('%Y%m%d')}.zip"
            }
        ]
...
要求：单张大图，整齐排列，每个表情清晰可辨，带中文文字，{base_prompt_suffix}""",
    ...
    def execute_step(self, step: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # 检查步骤类型
        step_type = step.get("type", "image_generation")
        
        if step_type == "image_generation":
            return self._execute_image_generation(step)
        elif step_type == "html_generation":
            return self._execute_html_generation(step)
        elif step_type == "packaging":
            return self._execute_packaging(step)
        elif step_type == "video_generation":
            return self._execute_video_generation(step)
        else:
            self.log(f"未知的步骤类型: {step_type}")
            return {
                "step_id": step["id"],
                "name": step["name"],
                "success": False,

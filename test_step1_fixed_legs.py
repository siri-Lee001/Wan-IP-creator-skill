...
3.  **姿态与补全**：
    *   主体保持**原始静态姿态**。
    *   **特别注意：角色必须四肢完整，长颈鹿有四条腿，不能缺少任何一条腿，所有视图都必须完整展示四肢！**
...
*   **A. 主体区：标准三视图（占据画面至少75%的宽度）**
    *   **内容**：严格生成且仅生成以下三个**正交视角**，在**同一水平基线上**从左至右**一字水平排列**：
        1.  **正面图 (FRONT VIEW)** - 必须完整展示四条腿
        2.  **侧面图 (SIDE VIEW)** - 通常为角色左侧视图，必须完整展示四条腿
        3.  **背面图 (BACK VIEW)** - 必须完整展示四条腿
    *   **关键要求**：
        *   **比例绝对对齐**：三个视图并排水平对齐，三个视图的**头顶、眼线、肩线、肘点、腰线、膝点、脚底**必须严格对齐。
        *   **细节绝对一致**：三个视图中的角色，必须是**同一个3D模型在三个正交视窗中的渲染结果**。所有细节（如口袋位置、图案、配饰形状）必须完全相同。
        *   **四肢绝对完整**：长颈鹿有四条腿，每个视图都必须完整展示所有四条腿，不能缺少任何一条！
        *   **背景**：统一使用上述定义的**白底+浅灰网格线**。"""
    
    print(f"提示词长度: {len(prompt)} 字符")
    print("正在调用万相API生成完整四肢的三视图...（预计耗时30-60秒）")
    ...
    if result and len(result) > 0:
        ...
        output_path = os.path.join(output_dir, "giraffe_3view_fixed_legs.jpg")
